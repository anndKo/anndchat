...
import { sanitizeInput, sanitizeFilename, isAllowedFileType } from "@/lib/security";
...
export const ChatArea = ({
  ...
  const deleteExpiredMessages = useCallback(async () => {
    ...
    if (expiredMessages.length > 0) {
      ...
      // Delete from database permanently - set is_deleted and clear content
      await supabase
        .from("messages")
        .update({ 
          is_deleted: true, 
      ...
      setMessages(prev => prev.map(m => 
        expiredIds.includes(m.id) 
          ? { ...m, is_deleted: true, content: null, file_url: null, file_name: null, file_type: null }
          : m
      ));
    }
  ...
  useEffect(() => {
    if (settings.auto_delete_24h) {
      ...
      const interval = setInterval(deleteExpiredMessages, 60000);
